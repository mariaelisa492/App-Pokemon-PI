...
const getPokeByName = async (name) => {
    try {
        ...
        if (searchPokeNameDB) {
            console.log('entro a db por name', searchPokeNameDB)
            let pokedbName = {
...
const objPokeApi = (poke) => {
    const objPokeapi =
    {
        ...
        types: poke.types.length < 2 ? [{ name: poke.types[0].type}] : [{ name: poke.types[0].type}, { name: poke.types[1].type}]
    };
    return objPokeapi
};
...
const postPokedb = async (pokeData) => {
    try {
        ...
        console.log('pokeTypedb', pokeTypedb)
